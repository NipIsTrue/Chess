...
public class Knight extends Piece {
	...
	public Knight(int x, int y, PieceType pieceType, PlayerType playerType) {
	...
	public void render(Graphics g) {
		if (playerType == PlayerType.WHITE) {
			...
			try {
				image = ImageIO.read(getClass().getResource("/resources/White_Knight.png"));
			...
			BufferedImage image = null;
			try {
				image = ImageIO.read(getClass().getResource("/resources/Black_Knight.png"));
				g.drawImage(image, x, y, null);
			} catch (IOException e) {
				e.printStackTrace();
			}
		...
		if (x <= 7 && y >= 0) {
			...
			if (tempCell.piece != null) {
				if (tempCell.piece.playerType != this.playerType && tempCell.piece.pieceType == PieceType.KING) {
					...
					System.out.println("Knight of color" + playerType.toString());
				}
			}
		}
