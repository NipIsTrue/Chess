...
public class Bishop extends Piece {
	...
	public Bishop(int x, int y, PieceType pieceType, PlayerType playerType) {
	...
	public void render(Graphics g) {
		if (playerType == PlayerType.WHITE) {
			...
			try {
				image = ImageIO.read(getClass().getResource("/resources/White_Bishop.png"));
			...
			BufferedImage image = null;
			try {
				image = ImageIO.read(getClass().getResource("/resources/Black_Bishop.png"));
				g.drawImage(image, x, y, null);
			} catch (IOException e) {
				e.printStackTrace();
			}
	...
	public void getValidMoves(Cell cell, Board board) {
		...
		for (int x = curX - 1; x >= 0; x--) {
			if (y <= 7) {
				...
				System.out.println("Highlighted cell " + tempCell.x + " " + tempCell.y);
